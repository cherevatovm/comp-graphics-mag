...
func initGLFW() (window *glfw.Window, err error) {
	...
	window, err = glfw.CreateWindow(scene.Width, scene.Height, "Lab 8", nil, nil)
	if err != nil {
		return nil, err
	}
...
func main() {
	...
	sc, err := scene.NewSceneTessLandscape(window)
	if err != nil {
		panic(err)
	}
	defer sc.Release()
	...
	window.SetKeyCallback(func(w *glfw.Window, key glfw.Key,
		...
		if key == glfw.KeyN && action == glfw.Press {
			sc.ShowNormals = !sc.ShowNormals
		}
		if key == glfw.KeyP && action == glfw.Press {
			sc.ShowPatch = !sc.ShowPatch
		}
